...
e.on("help", (str) => {
  let commands = {
    ...
    "list checks --up --down":
      "Show a list of all the active checks in the system, including their state, flags are optional",
    "more check info --{checkId}": "Show details of a specified check",
    "list logs":
      "Show a list of all the logs files available to be read (compressed only)",
...
cli.responders.listLogs = () => {
  ...
  ls.stdout.on("data", (dataObject) => {
    ...
    logFileNames.forEach((logFileName) => {
      if (typeof(logFileName) === 'string' && logFileName.legnth > 0 && logFileName.indexOf("-") > -1) {
